...
camera.position.set(0, 5, 8)  // temporary; overridden below after gallery is built
...
// ── Museum Scene (lights + floor + ceiling) ────────────────────────────────
const lights = buildMuseum(scene)

// ── Gallery (15 pedestals at fixed world positions) ────────────────────────
const { slots, hitObjects } = buildGallery(scene)

// ── Camera controller ──────────────────────────────────────────────────────
const cameraController = new CameraController(camera, orbit, slots)

// Set initial camera to slot 0 view
const { camPos: initCamPos, target: initTarget } = cameraController.viewForSlot(0)
camera.position.copy(initCamPos)
orbit.target.copy(initTarget)
orbit.update()
...
fetch(`${BASE}models/exhibits/manifest.json`)
  ...
  .then(async manifest => {
    ...
    function animate() {
      ...
      // cameraController.update calls orbit.update() internally when tweening;
      // only call orbit.update() here when idle (for damping).
      const tweening = cameraController.update(delta)
